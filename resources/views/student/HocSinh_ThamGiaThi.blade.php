...
.exam-sidebar { width:240px; min-width:240px; background:var(--white); border-right:1px solid var(--border); display:flex; flex-direction:column; overflow-y:auto; }
...
.modal-box2 { background:var(--white); border-radius:14px; padding:28px 32px; width:500px; max-width:90vw; box-shadow:0 8px 40px rgba(0,0,0,.2); }
.modal-box2 h2 { font-size:20px; font-weight:700; color:var(--navy); margin-bottom:8px; }
.modal-box2 p  { font-size:14px; color:var(--text-muted); line-height:1.6; margin-bottom:20px; }
.modal-stats2 { display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-bottom:24px; }
.stat2 { background:var(--bg); border-radius:8px; padding:14px; text-align:center; }
.stat2-num { font-size:24px; font-weight:700; color:var(--navy); }
.stat2-lab { font-size:12px; color:var(--text-muted); margin-top:2px; }
.modal-btns2 { display:flex; gap:10px; }
.btn-cancel2  { flex:1; background:var(--white); border:1.5px solid var(--border); color:var(--text); padding:11px; border-radius:8px; font-size:14px; font-weight:500; cursor:pointer; font-family:inherit; }
.btn-confirm2 { flex:1; background:var(--navy); color:#fff; border:none; padding:11px; border-radius:8px; font-size:14px; font-weight:600; cursor:pointer; font-family:inherit; }
</style>
</head>
<body>
...
<div id="screen-ready">
  <div class="ready-card">
    ...
    <div class="ready-stats">
      ...
      <div class="ready-stat">
        <div class="ready-stat-icon">📅</div>
        <div class="ready-stat-num" style="font-size:16px;padding-top:4px">
...
<div class="modal-bg" id="submit-modal" style="display:none">
  <div class="modal-box2">
    ...
    <div class="modal-stats2">
      <div class="stat2"><div class="stat2-num" id="m-answered">0</div><div class="stat2-lab">Đã trả lời</div></div>
      <div class="stat2"><div class="stat2-num" id="m-unanswered">0</div><div class="stat2-lab">Chưa trả lời</div></div>
      <div class="stat2"><div class="stat2-num" id="m-flagged">0</div><div class="stat2-lab">Đánh dấu</div></div>
      <div class="stat2"><div class="stat2-num" id="m-total">0</div><div class="stat2-lab">Tổng số câu</div></div>
    </div>
    <div class="modal-btns2">
      <button class="btn-cancel2"  id="btn-cancel-modal">Quay lại</button>
      <button class="btn-confirm2" id="btn-confirm-submit">Xác nhận nộp</button>
    </div>
  </div>
</div>
...
rawCauHoi.phan1_4pa.forEach(q => {
    questions.push({ type:'4pa', id:q.id, text:q.cau_hoi, opts:[q.a,q.b,q.c,q.d], partName:'Phần I – Trắc nghiệm 4 phương án' });
    answers.push(null);
});
rawCauHoi.phan2_ds.forEach(q => {
    questions.push({ type:'ds', id:q.id, text:q.cau_hoi, stmts:[q.md1,q.md2,q.md3,q.md4], partName:'Phần II – Trắc nghiệm Đúng/Sai' });
    answers.push([null,null,null,null]);
});
rawCauHoi.phan3_ngan.forEach(q => {
    questions.push({ type:'ngan', id:q.id, text:q.cau_hoi, partName:'Phần III – Trả lời ngắn' });
    answers.push('');
});
...
function buildGrid() {
    ...
    questions.forEach((_,i) => {
        ...
        b.className = 'qbtn' + (i===current?' current':'') + (isAns?' answered':'') + (flagged[i]?' flagged':'');
        b.textContent = i + 1;
        b.onclick = () => { current = i; render(); };
        grid.appendChild(b);
    });
    document.getElementById('answered-count').textContent = `Đã trả lời: ${done} / ${questions.length}`;
}
...
function render() {
    ...
    if (q.type === '4pa') {
        ...
        qContent = '<div class="options">' + q.stmts.map((stmt,idx) => {
        ...
        qContent = `<div class="short-ans-wrap">
          <p style="margin-bottom:8px;font-size:14px;color:var(--text-muted)">Nhập đáp án (tối đa 4 ký tự):</p>
          <input type="text" class="short-ans-input" maxlength="4" value="${answers[current]||''}"
                 oninput="inputNgan(this.value)" placeholder="Ví dụ: 12, 5.5">
        </div>`;
    }
    ...
    main.innerHTML = `
      ...
      ${qContent}
      <div class="q-nav">
        <button class="btn-nav" onclick="goPrev()" ${current===0?'disabled':''}>◀ Câu trước</button>
        <span class="q-progress">${answeredNum} / ${questions.length} đã trả lời</span>
        <button class="btn-nav" onclick="goNext()" ${current===questions.length-1?'disabled':''}>Câu tiếp ▶</button>
      </div>`;
