...
class TeacherController extends Controller
{
    ...
    public function deThiCauHoi(int $id): View
    {
        ...
        $in4PA = DB::select(
            "SELECT dtct.ID_DeThiChiTiet, q.ID_TracNghiem4PhuongAn,
             ...
             JOIN Cau_hoi_trac_nghiem_4_phuong_an q ON dtct.ID_TracNghiem4PhuongAn = q.ID_TracNghiem4PhuongAn
             WHERE dtct.ID_MaDeThi = ? ORDER BY q.ID_TracNghiem4PhuongAn", [$id]);

        $inDS = DB::select(
            "SELECT dtct.ID_DeThiChiTiet, q.ID_TracNghiemDungSai,
                    q.NoiDungCauHoi_TracNghiemDungSai, q.DapAn_TracNghiem4PhuongAn as DapAn
             FROM De_Thi_Chi_Tiet dtct
             JOIN Cau_hoi_trac_nghiem_dung_sai q ON dtct.ID_TracNghiemDungSai = q.ID_TracNghiemDungSai
             WHERE dtct.ID_MaDeThi = ? ORDER BY q.ID_TracNghiemDungSai", [$id]);

        $inNgan = DB::select(
            "SELECT dtct.ID_DeThiChiTiet, q.ID_TracNghiemTraLoiNgan,
                    q.NoiDungCauHoi_TracNghiemTraLoiNgan,
                    CONCAT(q.KiTuThu1CuaDapAn_TracNghiemTraLoiNgan, q.KiTuThu2CuaDapAn_TracNghiemTraLoiNgan, q.KiTuThu3CuaDapAn_TracNghiemTraLoiNgan, q.KiTuThu4CuaDapAn_TracNghiemTraLoiNgan) as DapAn
             FROM De_Thi_Chi_Tiet dtct
             JOIN Cau_hoi_tra_loi_ngan q ON dtct.ID_TracNghiemTraLoiNgan = q.ID_TracNghiemTraLoiNgan
             WHERE dtct.ID_MaDeThi = ? ORDER BY q.ID_TracNghiemTraLoiNgan", [$id]);

        $avail4PA = DB::select(
            "SELECT q.ID_TracNghiem4PhuongAn, q.NoiDungCauHoi_TracNghiem4PhuongAn, q.DapAn_TracNghiem4PhuongAn
             ...
             ORDER BY q.ID_TracNghiem4PhuongAn", [$deThi->ID_MaMon, $deThi->ID_MaKhoi, $id]);

        $availDS = DB::select(
            "SELECT q.ID_TracNghiemDungSai, q.NoiDungCauHoi_TracNghiemDungSai, q.DapAn_TracNghiem4PhuongAn as DapAn
             ...
             ORDER BY q.ID_TracNghiemDungSai", [$deThi->ID_MaMon, $deThi->ID_MaKhoi, $id]);
        ...
        $availNgan = DB::select(
            "SELECT q.ID_TracNghiemTraLoiNgan, q.NoiDungCauHoi_TracNghiemTraLoiNgan,
             ...
             ORDER BY q.ID_TracNghiemTraLoiNgan", [$deThi->ID_MaMon, $deThi->ID_MaKhoi, $id]);

        return view('teacher.GiaoVien_QuanLyDeThiCauHoi', compact('deThi', 'in4PA', 'inDS', 'inNgan', 'avail4PA', 'availDS', 'availNgan'));
    }
    ...
    public function tracNghiem4PA(Request $request): View
    {
        ...
        $chuDesAll = DB::select("SELECT cd.* FROM Chu_De cd WHERE cd.ID_NguoiTao = ? ORDER BY cd.NoiDung_ChuDe", [$teacherId]);
    ...
    public function diemDanh(Request $request): View
    {
        ...
        $lopHocs = DB::select("SELECT ID_LopHoc, TenLopHoc FROM Lop_hoc WHERE ID_Teacher = ? ORDER BY TenLopHoc", [$teacherId]);
        ...
        foreach ($lopHocs as $lop) {
            $studentsPerClass[$lop->ID_LopHoc] = DB::select(
                "SELECT u.ID_User, u.HoVaTen_User
                 FROM Lop_hoc_ThanhVien lv
                 JOIN `User` u ON lv.ID_Student = u.ID_User
                 WHERE lv.ID_LopHoc = ? ORDER BY u.HoVaTen_User",
                [$lop->ID_LopHoc]
            );
        }
    ...
    public function diemDanhStore(Request $request): RedirectResponse
    {
        ...
        DB::table('Diem_danh')->insert([
            ...
            'ThoiGianKetThuc_DiemDanh'  => $data['ThoiGianKetThuc_DiemDanh'] ? $ngay . ' ' . $data['ThoiGianKetThuc_DiemDanh'] . ':00' : null,
    ...
    public function diemDanhUpdate(Request $request, int $id): RedirectResponse
    {
        ...
        $dd = DB::selectOne(
            "SELECT dd.* FROM Diem_danh dd
             JOIN Lop_hoc l ON dd.ID_LopHoc = l.ID_LopHoc
             WHERE dd.ID_DiemDanh = ? AND l.ID_Teacher = ?", [$id, $teacherId]
        );
        abort_if(!$dd, 403);
        ...
        DB::table('Diem_danh')->where('ID_DiemDanh', $id)->update([
            'TrangThaiBuoiHoc_DiemDanh'  => in_array($trangThai, ['scheduled', 'in_progress', 'completed', 'cancelled']) ? $trangThai : $dd->TrangThaiBuoiHoc_DiemDanh,
    ...
    public function kyThi(Request $request): View
    {
        ...
        $lopHocsAll = DB::select("SELECT ID_LopHoc, TenLopHoc, ID_MonHoc, ID_KhoiLop FROM Lop_hoc WHERE ID_Teacher = ? ORDER BY TenLopHoc", [$teacherId]);
        $chuDesAll  = DB::select("SELECT ID_ChuDe, NoiDung_ChuDe, ID_MonHoc, ID_KhoiLop FROM Chu_De WHERE ID_NguoiTao = ? ORDER BY NoiDung_ChuDe", [$teacherId]);
        $deThisAll  = DB::select("SELECT ID_MaDeThi, TenDeThi, ID_MaMon, ID_MaKhoi FROM De_Thi WHERE ID_NguoiTao = ? ORDER BY TenDeThi", [$teacherId]);
        
        $khoiLops = DB::select(
            "SELECT DISTINCT k.ID_KhoiLop, k.Ten_KhoiLop
             FROM Lop_hoc l JOIN Khoi_lop k ON l.ID_KhoiLop = k.ID_KhoiLop
             WHERE l.ID_Teacher = ? ORDER BY k.Ten_KhoiLop", [$teacherId]);

        $monHocs = DB::select(
            "SELECT DISTINCT m.ID_MonHoc, m.Ten_MonHoc
             FROM Lop_hoc l JOIN Mon_Hoc m ON l.ID_MonHoc = m.ID_MonHoc
             WHERE l.ID_Teacher = ? ORDER BY m.Ten_MonHoc", [$teacherId]);
    ...
    public function kyThiStore(Request $request): RedirectResponse
    {
        ...
        $lop = DB::table('Lop_hoc')->where('ID_LopHoc', $data['ID_LopHoc'])->where('ID_Teacher', $teacherId)->first();
        abort_if(!$lop, 403);
    ...
    public function kyThiUpdate(Request $request, int $id): RedirectResponse
    {
        ...
        $kt = DB::selectOne("SELECT kt.ID_KyThi FROM Ky_thi kt JOIN Lop_hoc l ON kt.ID_LopHoc = l.ID_LopHoc WHERE kt.ID_KyThi = ? AND l.ID_Teacher = ?", [$id, $teacherId]);
        abort_if(!$kt, 403);
    ...
    public function donXinNghi(Request $request): View
    {
        $teacherId = session('auth.id');
        $donNghis = DB::select(
            "SELECT dxn.*, u.HoVaTen_User as ten_hoc_sinh, l.TenLopHoc, dd.NgayHoc_DiemDanh
    ...
    {
        $teacherId = session('auth.id');
        $don = DB::selectOne("SELECT dxn.ID_DonXinNghi FROM Don_xin_nghi dxn JOIN Lop_hoc l ON dxn.ID_LopHoc = l.ID_LopHoc WHERE dxn.ID_DonXinNghi = ? AND l.ID_Teacher = ?", [$id, $teacherId]);
        abort_if(!$don, 403);
        ...
        DB::table('Don_xin_nghi')->where('ID_DonXinNghi', $id)->update(['TrangThai_DonXinNghi' => $trangThai]);
    ...
    // =========================================================================
    // KHU VỰC ĐÃ MERGE: THÔNG TIN CÁ NHÂN & ĐỔI MẬT KHẨU MỚI NHẤT
    // =========================================================================

    // 1. Xem thông tin (Sử dụng lệnh truy vấn chi tiết từ code cũ)
    public function thongTin(Request $request): View
    {
        ...
        // Lưu ý: Đổi lại tên View cho khớp nếu lúc trước bạn lưu là GiaoVien_ThongTin
        return view('teacher.GiaoVien_ThongTin', compact('teacher'));
    }
    ...
    public function editProfile(Request $request): View 
    {
        ...
        $khoiLops = DB::table('Khoi_Lop')->get();
        return view('teacher.GiaoVien_ChinhSua', compact('teacher', 'monHocs', 'khoiLops'));
    }
    ...
    public function capNhatThongTin(Request $request): RedirectResponse
    {
        ...
        $data = $request->validate([
            ...
            'PhuTrachMon_User'      => 'nullable|integer',
            'PhuTrachKhoi_User'     => 'nullable|integer',
        ]);

        DB::table('User')->where('ID_User', $teacherId)->update([
            'HoVaTen_User'          => $data['HoVaTen_User'],
            'NgayThangNamSinh_User' => $data['NgayThangNamSinh_User'],
            'EmailCaNhan_User'      => $data['EmailCaNhan_User'],
            'SoDienThoai_User'      => $data['SoDienThoai_User'],
    ...
    // 5. Xử lý logic Đổi mật khẩu cực kỳ an toàn
    public function xuLyDoiMatKhau(Request $request): RedirectResponse
    {
        $request->validate([
        ...
        // Xử lý thông minh cho cả mật khẩu MD5, Bcrypt và chữ thường
        if (md5($request->old_password) === $teacher->Pass_User) {
            $isPasswordCorrect = true;
        } elseif (Hash::info($teacher->Pass_User)['algoName'] === 'bcrypt') {
            $isPasswordCorrect = Hash::check($request->old_password, $teacher->Pass_User);
        } elseif ($request->old_password === $teacher->Pass_User) {
            $isPasswordCorrect = true;
        }
        ...
        // Cập nhật pass mới với Bcrypt chuẩn Laravel
        DB::table('User')->where('ID_User', $teacherId)->update([
            'Pass_User' => Hash::make($request->new_password)
        ]);
